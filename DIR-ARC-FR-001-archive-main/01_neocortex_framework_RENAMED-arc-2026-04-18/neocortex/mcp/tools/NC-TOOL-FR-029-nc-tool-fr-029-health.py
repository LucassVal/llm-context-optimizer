...
def register_tool(mcp) -> None:
    ...
    def neocortex_health(
        ...
        Aes disponveis:
          server.status       uptime, tools, transport
          server.tools_count  contagem de tools e aes
          agent.health        /health de sub-server (agent_role ou port)
          agent.health_all    health check de todos os sub-servers
          metrics.live        mtricas em tempo real
          log.tail            ltimas N linhas do log (lines=50 default)
          log.errors          apenas entradas de erro do log
          log.search          busca por pattern no log
          log.purge           apagar logs com mais de N dias (days=7)

        Args:
            action:     Ao desejada
            agent_role: Role do agente para agent.health (courier|engineer|guardian)
            port:       Porta alternativa para agent.health
            lines:      Nmero de linhas para log.tail (default: 50)
            pattern:    Pattern para log.search
            days:       Dias de TTL para log.purge (default: 7)
        """
        ...
        #  metrics.live 
            ...
            if ms is None:
                return {"success": False, "error": "MetricsStore indisponvel."}
            try:
                summary = ms.get_summary() if hasattr(ms, "get_summary") else {}
                return {"success": True, "action": action, "metrics": summary, "timestamp": datetime.now().isoformat()}
            except Exception as e:
