...
FR-032  MCP Tool: neocortex_push_notification
...
def _get_event_bus():
    ...
    try:
        ...
        # R09: mdulos com hfen no suportam import_module  usar spec_from_file_location
        event_bus_path = (
...
def register_tool(mcp) -> None:
    ...
    def neocortex_push_notification(
        ...
        Aes disponveis:
          push.send  envia notificao desktop
          push.status  verifica disponibilidade (notifypy + flag)

        Args:
            action:      'push.send' ou 'push.status'
            title:       ttulo da notificao (obrigatrio para send)
            message:     corpo da mensagem (obrigatrio para send)
            urgency:     nvel de urgncia: low, normal, high, critical (default normal)

        Retorna:
            Dict com campos success, sent, reason, detail, etc.
        """
        timestamp = datetime.now().isoformat()
        ...
        if action == "push.status":
            return {
                ...
                "event_bus_available": _get_event_bus() is not None,
            }
