...
def _scan_lobes(root: Path) -> list:
    """Escaneia 02_memory_lobes/ e extrai metadata de cada lobe."""
    lobes_root = root.parent / "02_memory_lobes" if (root.parent / "02_memory_lobes").exists() else root / "02_memory_lobes"
    results = []
    if not lobes_root.exists():
        # Tenta procurar em qualquer lugar do projeto
        candidates = list(root.parent.glob("**/02_memory_lobes"))
        if not candidates:
            return results
        lobes_root = candidates[0]
