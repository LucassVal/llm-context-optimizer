...
"""---
_genealogy:
  injected_at: '2026-04-16T00:24:01.572663'
...
# -*- coding: utf-8 -*-
"""
Povoamento exaustivo dos Lobos do NeoCortex com contedo especfico do projeto.
Analisa a estrutura do projeto e preenche cada Lobo com informaes relevantes.
"""
...
def get_lobe_mapping():
    """Retorna mapeamento de Lobos para mdulos/reas do projeto."""
    return {
        "NC-LBE-FR-ARCHITECTURE-001.mdc": {
            "module": "architecture",
            "description": "Arquitetura do framework, padres, componentes",
        ...
        "NC-LBE-FR-DEVELOPMENT-001.mdc": {
            "module": "development",
            "description": "Prticas de desenvolvimento, cdigo fonte, convenes",
            "target_dirs": [
        ...
        "NC-LBE-FR-KNOWLEDGE-001.mdc": {
            "module": "knowledge",
            "description": "Conhecimento geral, graph, relaes",
        ...
        "NC-LBE-FR-SECURITY-001.mdc": {
            "module": "security",
            "description": "Segurana, autenticao, permisses",
        ...
        "NC-LBE-FR-DOCUMENTATION-001.mdc": {
            ...
            "description": "Documentao, guias, referncias",
        ...
        "NC-LBE-FR-INTEGRATION-001.mdc": {
            "module": "integration",
            "description": "Integrao com outros sistemas",
        ...
        "NC-LBE-FR-MONITORING-001.mdc": {
            "module": "monitoring",
            "description": "Monitoramento, logs, mtricas",
        ...
        "NC-LBE-FR-PERFORMANCE-001.mdc": {
            "module": "performance",
            "description": "Performance, benchmarks, otimizao",
        ...
        "NC-LBE-FR-CORE-001.mdc": {
            "module": "core",
            "description": "Ncleo do framework",
            "target_dirs": ["neocortex_framework/neocortex/core/"],
        ...
        "NC-LBE-FR-PROFILES-001.mdc": {
            "module": "profiles",
            "description": "Perfis de configurao",
        ...
        "NC-LBE-FR-PULSE-001.mdc": {
            "module": "pulse",
            "description": "Pulse Scheduler, execuo peridica",
            "target_dirs": ["neocortex_framework/neocortex/core/"],
...
def analyze_directory(dir_path, pattern="**/*.py"):
    """Analisa um diretrio especfico e retorna informaes."""
    path = (
...
def extract_python_info(file_path):
    """Extrai informaes bsicas de um arquivo Python."""
    try:
        # file_path  relativo ao PROJECT_ROOT
        abs_path = (
...
def generate_lobe_content(lobe_name, mapping, project_analysis, dir_analysis):
    """Gera contedo especfico para um Lobo baseado na anlise."""
    ...
    content += "## Checkpoints\n"
    content += f"- CP-{module.upper()}-001: Criao inicial\n"
    content += f"- CP-{module.upper()}-002: Povoamento automtico\n\n"

    # Adicionar anlise de diretrio
    if dir_analysis.get("exists"):
        content += "## Anlise do Diretrio\n"
        ...
        if python_files:
            content += "\n### Anlise de Cdigo Python\n"
            for file in python_files[:5]:  # Analisar at 5 arquivos
                ...
                if "error" not in py_info:
                    ...
                    content += f"- **Funes**: {len(py_info['functions'])}\n"
    ...
    # Adicionar informaes do projeto
    if project_analysis:
        content += "\n## Contexto do Projeto\n"
        content += (
    ...
    content += "\n## Notas de Povoamento\n"
    content += "- Povoado automaticamente por OpenCode\n"
    content += f"- Data: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n"
    content += f"- Foco da anlise: {', '.join(mapping.get('analysis_focus', []))}\n"

    return content


def update_lobe_with_content(lobe_name, new_content):
    """Atualiza um Lobo existente com novo contedo."""
    ...
    # Combinar contedo atual com novo (adicionar seo de anlise)
    current_content = lobe_info["content"]

    # Verificar se j existe seo de anlise
    if "## Anlise do Diretrio" in current_content:
        # Substituir seo existente
        lines = current_content.split("\n")
        new_lines = []
        in_analysis_section = False
        for line in lines:
            if line.startswith("## Anlise do Diretrio"):
                in_analysis_section = True
                new_lines.append("## Anlise do Diretrio (atualizada)")
        ...
        # Adicionar nova anlise no final
        updated_content = "\n".join(new_lines)
        # Extrair apenas a seo de anlise do novo contedo
        analysis_start = new_content.find("## Anlise do Diretrio")
        if analysis_start != -1:
            analysis_section = new_content[analysis_start:]
            # Inserir antes da primeira seo aps a anlise
            updated_content = updated_content.replace(
                "## Anlise do Diretrio (atualizada)", analysis_section
            )
    else:
        # Adicionar nova anlise ao final
...
def create_kg_relations(lobe_name, mapping):
    """Cria relaes no Knowledge Graph para o Lobo."""
    ...
    for entity in entities:
        ...
        if not entity_result.get("success"):
            ...
            # Continuar mesmo se falhar (pode j existir)

    # Criar relao com o crtex central
...
def main():
    ...
    for lobe_name, mapping in lobe_mapping.items():
        ...
        lobe_info = lobe_service.get_lobe(lobe_name)
        if not lobe_info.get("exists"):
            print("  [SKIP] Lobe no encontrado, criando...")
            # Criar lobe bsico primeiro
            basic_content = (
        ...
        if update_success:
            print("  [OK] Lobo atualizado com contedo especfico")
            ...
            if kg_success:
                print("  [OK] Relaes KG criadas")
            else:
                print("  [WARN] Falha ao criar relaes KG")

            successes += 1
        else:
            print("  [ERROR] Falha ao atualizar lobe")
    ...
    print(f"- Anlise do projeto: {'Sim' if project_analysis else 'No'}")
    print(
