...
$outputDir = "DIR-RES-CC-001-claude-leak-workzone\analysis-session-b\temp"
foreach ($term in $terms) {
    $files = Get-ChildItem -LiteralPath $srcPath -Recurse -Include *.ts, *.tsx | Select-String -Pattern $term | Select-Object -Unique Path
    $count = $files.Count
