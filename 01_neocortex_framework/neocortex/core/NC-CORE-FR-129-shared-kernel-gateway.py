...
class ConstitutionGateway:
    ...
    # ═════════════════════════════════════════════════════════
    # NC-DS-210: HOOK REGISTRY INTEGRATION (v2 — corrigido)
    # Hooks inline (sem dependencia externa). Complementa hardcoded.
    # ═════════════════════════════════════════════════════════
    ...
    def _check_registry_hooks(
        ...
        hooks = {
            "file_write": [
                {"name": "R01-naming-convention"},
                {"name": "R04-atomic-locks"},
                {"name": "R51-fail-fast"},
                {"name": "R112-yaml-validate"},
                {"name": "R113-mdc-validate"},
                {"name": "R114-secret-scan"},
            ],
            "action_execute": [{"name": "R21-zero-assumptions"}],
            "file_create": [{"name": "R42-3w-check"}],
        ...
        for _event, hook_list in hooks.items():
            for hook in hook_list:
                ...
                if name == "R01-naming-convention" and tp and tp.exists():
                    fname = tp.name
                    ok = (
                        fname.startswith("NC-")
                        or fname.startswith("DIR-")
                        or fname.startswith(".")
                    )
                    report["checks"]["hook_R01"] = ok
                    if not ok:
                        report["violations"].append(
                            f"R01(hook): {fname} nao segue NC-<TIPO>-<SIGLA>-<NUM>"
                        )

                elif name == "R21-zero-assumptions":
                    report["checks"]["hook_R21"] = True

                elif name == "R51-fail-fast" and tp and tp.exists():
                    if tp.suffix == ".py":
                        try:
                            import subprocess as _sp

                            r = _sp.run(
                                ["python", "-m", "py_compile", str(tp)],
                                capture_output=True,
                                text=True,
                                timeout=10,
                            )
                            ...
                            ok = True
                        report["checks"]["hook_R51"] = ok
                        if not ok:
                            report["violations"].append(
                                f"R51(hook): {tp.name} nao compila"
                            )
                    elif tp.suffix in (".yaml", ".yml"):
                        try:
                            ...
                            report["violations"].append(
                                f"R51(hook): {tp.name} YAML invalido"
                            )
                    ...
                    and tp
                    and tp.exists()
                    and tp.suffix in (".yaml", ".yml")
                ):
                    try:
                        import yaml as _yaml

                        _yaml.safe_load(tp.read_text(encoding="utf-8", errors="ignore"))
                        report["checks"]["hook_R112"] = True
                    except Exception:
                        report["checks"]["hook_R112"] = False
                        report["violations"].append(
                            f"R112(hook): {tp.name} YAML invalido"
                        )
                    ...
                    report["checks"]["hook_R113"] = ok
                    if not ok:
                        report["violations"].append(
                            f"R113(hook): {tp.name} sem frontmatter YAML"
                        )

                elif name == "R114-secret-scan" and tp and tp.exists():
                    content = tp.read_text(encoding="utf-8", errors="ignore")[
                        :5000
                    ].lower()
                    ok = not any(
                        p in content for p in ["sk-", "api_key", "token=", "password="]
                    )
                    report["checks"]["hook_R114"] = ok
                    if not ok:
                        report["violations"].append(
                            f"R114(hook): {tp.name} pode conter secrets"
                        )

                elif name == "R117-ssot-header":
