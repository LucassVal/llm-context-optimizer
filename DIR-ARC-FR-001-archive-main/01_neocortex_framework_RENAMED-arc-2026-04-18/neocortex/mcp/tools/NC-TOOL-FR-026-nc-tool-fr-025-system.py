...
def register_tool(mcp) -> None:
    ...
    def neocortex_system(
        ...
        Args:
            action:            Ao desejada
            key:               Chave para config.diff, config.set_constraint, config.set_agent_backend, ou nome da task customizada
            value:             Valor para config.diff, config.set_constraint, config.set_agent_backend
            task_name:         Nome da task para pulse.force (ex: "pruning", "consolidation")
            schedule_interval: Intervalo em segundos para pulse.schedule_custom (default: 300)
        """
