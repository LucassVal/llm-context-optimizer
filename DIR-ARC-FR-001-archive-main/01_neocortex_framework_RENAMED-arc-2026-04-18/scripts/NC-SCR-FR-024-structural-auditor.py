...
layer: "infra"
type: "SCR"
tags: ["auditor", "structural", "naming", "kgs", "nc-ds-051"]
hash: "auto-generated"
---"""
...
ALLOWED_TYPES = {
    "TOOL", "SCR", "DOC", "TODO", "LBE", "CFG", "SEC", "SOP",
    "ARC", "BAK", "BOOT", "TST", "PROMPT", "APP", "NAM", "AUD", "LED", "CTX", "HK", "VAL", "WKR", "SVC", "UTL", "ADP", "REV"
}
...
TYPE_TO_DIR = {
    ...
    "LBE": "01_neocortex_framework/lobes", # Or 02_memory_lobes
...
def check_naming_convention(filepath: Path) -> dict:
    ...
    if filename in ["__init__.py", "neocortex_config.yaml", "neocortex_config_dev.yaml", "README.md", "vector_engine.py", "logging_config.py"]:
...
def check_directory_conformance(filepath: Path, tipo: str) -> dict:
    ...
    if not expected_dir:
        return {"conforms": True, "warning": f"Sem diretrio mapeado para tipo '{tipo}' na regra rgida."}
    ...
    if expected_dir.replace("/", "\\") in str(rel_path) or expected_dir in str(rel_path.as_posix()):
        return {"conforms": True}

    # Special exceptions due to refactoring
    if tipo == "CFG" and "neocortex/core/config" in str(rel_path.as_posix()):
        return {"conforms": True}
    if tipo == "LBE" and "02_memory_lobes" in str(rel_path.as_posix()):
        return {"conforms": True}
...
def check_content_hints(filepath: Path, tipo: str) -> dict:
    ...
    if tipo == "TOOL" and "def register_tool" not in content and "class" not in content:
        hints.append("Arquivo TOOL parece no conter 'def register_tool' ou a lgica MCP T0")
    ...
    if non_conforming:
        for item in non_conforming:
            ...
            if 'directory_error' in item:
                md_content += f"- **Erro de Diretrio / Fallback:** {item['directory_error']}\n"
            md_content += "\n"
    else:
        md_content += " Nenhuma no-conformidade encontrada. Arquitetura Diamante Pronta.\n\n"
