...
### 5. KISS / DRY / YAGNI / KARPATHY SKILLS (PODA CONTÍNUA)
  ...
  YAGNI: Se não resolve problema ativo com ticket, ADIE.
  KARPATHY SKILLS: Think before coding (recuse se confuso). Simplicity First. Surgical Changes (NÃO refatore código adjacente que não está quebrado). Goal-Driven (verifique o sucesso a cada passo).
...
### 7. DDD — ARQUITETURA PARENT-CHILD E REGIÕES CEREBRAIS
  6 Regiões Cerebrais: $FRONTAL (Roadmap/Decisão), $TEMPORAL (Semântica/KG), $PARIETAL (Integração/MCP), $OCCIPITAL (Padrões/Naming), $CEREBELO (Motor/Guardian), $HIPOCAMPO (Sessões/Savepoints).
...
### 10. COMPLIANCE TARGETS (R58) E AUTOCICLO
  Score mínimo: 80% | Alvo: 92%
  AUTOCICLO OBRIGATÓRIO: Plan → Code → Lint (Ruff/Mypy) → Compile → Restart MCP → Test. Nunca pule etapas.
...
### 13. INTEGRIDADE DO SISTEMA E SEMÂNTICA (R112-R123)
  R112-R115: YAML Validate, MDC Header, Secret Scan, Dead Code Audit.
  R116: Memory Auto-Record — toda ação registrada (Gateway)
  R117: SSOT Header OBRIGATÓRIO (Ex: `NC-SSOT | timestamp | ... | RAC:ON 3W:ON KISS:ON`)
  R118: Domain Routers — index semântico 2 níveis (FR-166)
  R119: Semantic Guardian — vigia da saúde semântica (FR-167, 300s)
  R120: Semantic Boot (DECAY) — sequência ULQ→TAGS→PREP + decay (7d Stale, 30d Removed)
...
### 18. ROTINAS DE VALIDAÇÃO (EXECUÇÃO OBRIGATÓRIA)
Antes de declarar %DONE ou submeter um Handoff, você **deve** validar os arquivos afetados de forma autônoma:

  1. PYTHON LINT (Ruff): `python -m ruff check <arquivos_modificados>` (Falha = abortar handoff)
  2. PYTHON TYPES (Mypy/Pyright): `python -m mypy <arquivos_modificados>` e `npx pyright <arquivos_modificados>`
  3. YAML SYNTAX: Utilizar `yaml.safe_load()` em script Python rápido ou acionar `NC-SCR-FR-009-sanitize-all-yamls.py`.
  4. MD/MDC LINKS: Garantir que URIs e Paths não estão quebrados e respeitam o @SSOT.
  5. SECRETS: Executar regex ou script rápido para garantir ausência de senhas/API Keys em texto puro.
