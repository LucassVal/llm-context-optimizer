...
if %errorlevel%==0 (
    ...
    start "NeoCortex MCP" /min cmd /c "cd /d "%FW%" && set PYTHONPATH=%FW% && set NEOCORTEX_PROJECT_ROOT=%FW% && set NEOCORTEX_LOG_LEVEL=ERROR && "%PYTHON%" -m neocortex.mcp.server --transport stdio > nul 2>&1"
    set MCP_STARTED=1
