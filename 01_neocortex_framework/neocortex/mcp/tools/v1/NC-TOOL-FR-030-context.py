...
"""---
_genealogy:
  injected_at: '2026-04-16T00:24:01.230152'
  injected_by: NC-SCR-FR-075-genealogy-injector.py
  version: '1.0'
topology: neocortex-other
level: 0
parent_ssot: NC-TODO-FR-001
related_ssot:
  - NC-NAM-FR-001
  - '@BOOT'
  - '@SSOT'
  - NC-TOOL-FR-030-context
  - NC-BOOT-FR-001
tags:
  - neocortex-other
  - level-0
...
def register_tool(mcp) -> None:
    ...
    def neocortex_context(
        ...
        """Economia de tokens e gestão de contexto do NeoCortex.

        Actions: context.budget_status, context.window_used, context.estimate,
                 context.compress, context.force_compress, context.smart_prune,
                 session.summarize, session.handoff, cache.stats
        """
        ...
        # T-10: context.force_compress — prune agressivo + audit persistence
        elif action == "context.force_compress":
            ld = _ledger()
            pruned = 0
            if ld:
                try:
                    result = ld.prune_context(max_entries=max_entries) if hasattr(ld, "prune_context") else {"pruned": 0}
                    pruned = result.get("pruned", 0)
                except Exception:
                    pass
            uptime_s = int(time.time() - _SESSION_START)
            compress_record = {
                "action": "context.force_compress",
                "timestamp": datetime.now().isoformat(),
                "entries_pruned": pruned,
                "max_entries_kept": max_entries,
                "session_uptime_s": uptime_s,
                "model": model,
            }
            # Persiste em audit-logs
            try:
                cfg = _config()
                if cfg:
                    from pathlib import Path as _P
                    audit_dir = _P(cfg.project_root) / "DIR-DS-002-audit-logs"
                    audit_dir.mkdir(parents=True, exist_ok=True)
                    today = datetime.now().strftime("%Y-%m-%d-%H%M")
                    import json as _json
                    with open(audit_dir / f"NC-CTX-DS-{today}-compress.json", "w", encoding="utf-8") as fh:
                        _json.dump(compress_record, fh, indent=2)
            except Exception:
                pass
            return {"success": True, "action": action, "pruned": pruned, "compress_record": compress_record}
