...
# Add core directory to Python path for security utilities import
...
def tool_peers(action: str, peer_id: str = "", state_data: str = "") -> Dict[str, Any]:
    """
    ...
    # Inicializa estrutura de peers se nao existir
    memory_cortex = ledger.get("memory_cortex", {})
    if "peers" not in memory_cortex:
        memory_cortex["peers"] = []
        ledger["memory_cortex"] = memory_cortex

    peers = memory_cortex.get("peers", [])
    ...
    if action == "discover":
        ...
        return {
            ...
            "peers": peers,
            "count": len(peers),
            "message": f"{len(peers)} peers disponiveis",
        }

    elif action == "sync_state":
        if not peer_id:
            return {"success": False, "error": "peer_id e obrigatorio"}
        ...
        sync_record = {
            ...
            "timestamp": "auto_generated",
            "state_synced": True,
            "data_preview": state_data[:100] if state_data else "vazio",
        }

        # Adiciona peer se nao existir
        ...
        return {
            ...
            "message": f"Estado sincronizado com peer '{peer_id}'",
        }

    elif action == "resolve_conflict":
        # Simulacao de resolucao de conflito
        if not peer_id:
            return {"success": False, "error": "peer_id e obrigatorio"}
        ...
        conflict_resolution = {
            ...
            "timestamp": "auto_generated",
            "message": "Conflito resolvido via merge automatico",
        }

        return {
            "success": True,
            "resolution": conflict_resolution,
            "message": f"Conflito com peer '{peer_id}' resolvido",
        }
...
def tool_security(
    ...
    # Inicializa estrutura de seguranca se nao existir
    memory_cortex = ledger.get("memory_cortex", {})
    ...
    if action == "validate_access":
        ...
        if not current_user_id:
            ...
            if not current_user_id:
                return {
                    "success": False,
                    "error": "current_user_id nao especificado e nao encontrado no ledger",
                }
        ...
        if not target_user_id:
            return {
                "success": False,
                "error": "target_user_id ou resource e obrigatorio",
            }
        ...
        return {
            ...
            "message": f"Validacao de acesso concluida: {access_validation['message']}",
        }

    elif action == "audit_changes":
        # Retorna historico de auditoria
        ...
        return {
            ...
            "message": f"{len(recent_audits)} entradas recentes no log de auditoria",
        }

    elif action == "encrypt_sensitive":
        if not resource:
            return {"success": False, "error": "resource e obrigatorio"}

        # Simulacao de criptografia
        encryption_record = {
            "resource": resource,
            "timestamp": "auto_generated",
        ...
        return {
            ...
            "message": f"Recurso '{resource}' marcado para criptografia",
        }
