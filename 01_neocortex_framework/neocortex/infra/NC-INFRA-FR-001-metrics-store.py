"""---
@Module  mcp _genealogy:   injected_at: '2026-04-16T00:23:59.58
---
"""
...
class MetricsStore:
    ...
    def _init_duckdb(self):
        ...
        # NEOCORTEX_METRICS_MODE=memory skips file entirely (multi-process safe)
        if os.environ.get("NEOCORTEX_METRICS_MODE", "").lower() == "memory":
            import duckdb
            self.conn = duckdb.connect(":memory:")
            logger.info("DuckDB :memory: mode (NEOCORTEX_METRICS_MODE=memory)")
            return
    ...
    def _create_tables(self):
        ...
        if self.backend == MetricsBackend.DUCKDB:
            # DuckDB uses TEXT PRIMARY KEY (UUID)
            self.conn.execute("""
    ...
    def insert_metric(
        ...
        try:
            ...
            self.conn.execute(
                """
            INSERT INTO metrics
            (id, metric_id, metric_type, value, timestamp, tags_json, metadata_json)
            VALUES (?, ?, ?, ?, ?, ?, ?)
            """,
                (
                    str(uuid.uuid4()),
