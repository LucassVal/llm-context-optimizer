...
def register_tool(mcp):
    ...
    def tool_config(action: str, key: str = "", value: str = "") -> Dict[str, Any]:
        ...
        Actions: get_config, set_model, list_models, set_constraint, list_constraints, set_agent_backend"""
        # Get config service
        ...
        if action == "get_config":
        ...
        elif action == "set_agent_backend":
            if not key or not value:
                return {
                    "success": False,
                    "error": "key (role) and value (backend config JSON) are required",
                }
            ...
            config = get_config()
            config.set(f"llm.agent_backends.{key}", backend_config)
