"""---
_genealogy:
  injected_at: '2026-04-16T00:24:00.552347'
...
def register_tool(mcp):
    ...
    def tool_report(
        action: str, format: str = "markdown", days: int = 7, save_to_audit: bool = False
    ) -> Dict[str, Any]:
        """Gera relatórios de métricas NeoCortex (markdown, csv, json).

        Actions: generate_daily_summary, generate_cost_report, generate_agent_report, get_metrics_stats
        Param save_to_audit=True → persiste em DIR-DS-002-audit-logs/ como NC-RPT-DS-{date}-{action}.md
        """
        result: Dict[str, Any]
        if action == "generate_daily_summary":
            result = _generate_daily_summary(metrics_store, format, days)
        elif action == "generate_cost_report":
            result = _generate_cost_report(metrics_store, format, days)
        elif action == "generate_agent_report":
            result = _generate_agent_report(metrics_store, format, days)
        elif action == "get_metrics_stats":
            result = _get_metrics_stats(metrics_store, format)
        else:
            return {
                "success": False,
        ...
        # T-16: persistir em audit-logs se solicitado
        if save_to_audit and result.get("success") and result.get("report"):
            try:
                from ...config import get_config as _get_cfg
                _cfg = _get_cfg()
                audit_dir = Path(_cfg.project_root) / "DIR-DS-002-audit-logs"
                audit_dir.mkdir(parents=True, exist_ok=True)
                today = datetime.now().strftime("%Y-%m-%d")
                action_slug = action.replace("_", "-")
                ext = {"markdown": "md", "csv": "csv", "json": "json"}.get(format, "md")
                audit_filename = f"NC-RPT-DS-{today}-{action_slug}.{ext}"
                audit_path = audit_dir / audit_filename
                with open(audit_path, "w", encoding="utf-8") as fh:
                    fh.write(result["report"])
                result["audit_path"] = str(audit_path)
                logger.info(f"Report saved to audit-logs: {audit_path}")
            except Exception as exc:
                result["audit_save_error"] = str(exc)
        return result
