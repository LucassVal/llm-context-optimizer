...
"""---
_genealogy:
  injected_at: '2026-04-16T00:24:01.806262'
  injected_by: NC-SCR-FR-075-genealogy-injector.py
  version: '1.0'
topology: neocortex-other
level: 0
parent_ssot: NC-CTX-FR-001-cortex-central
tags:
  - neocortex-other
  - level-0
  - python
...
2. Move arquivos utilitrios para archive apropriado
...
def create_legacy_lobe():
    ...
    # Contedo do Lobo LEGACY
    content = (
        """# NC-LBE-FR-LEGACY-001

Gesto de arquivos e diretrios legados no projeto TURBOQUANT_V42.

## Propsito
Documentar e gerenciar arquivos legados, utilitrios de desenvolvimento, 
e estruturas antigas que no fazem parte do framework NeoCortex consolidado.
...
## Checkpoints
- CP-LEGACY-001: Criao inicial do Lobo LEGACY
- CP-LEGACY-002: Inventrio de arquivos legados
- CP-LEGACY-003: Movimentao para archive
- CP-LEGACY-004: Limpeza da raiz

## Inventrio de Arquivos Legados

### Arquivos na Raiz (pr-saneamento)
| Arquivo | Descrio | Status | Ao Recomendada |
|---------|-----------|--------|------------------|
| clean_security.py | Limpeza do mdulo security.py | Obsoleto | Mover para archive |
| extract_all_tools.py | Extrao de ferramentas MCP | Obsoleto | Mover para archive |
| extract_tools_final.py | Extrao final de ferramentas | Obsoleto | Mover para archive |
| extract_tools_robust.py | Extrao robusta de ferramentas | Obsoleto | Mover para archive |
| fix_indentation.py | Correo de indentao | Obsoleto | Mover para archive |
| update_phase3_progress.py | Atualizao de progresso | Obsoleto | Mover para archive |
| update_ledger_status.py | Atualizao de status do ledger | Obsoleto | Mover para archive |
| verify_mcp.py | Verificao de MCP | Obsoleto | Mover para archive |
| update_antigravity_confirmation.py | Confirmao Antigravity | Obsoleto | Mover para archive |
| add_root_sanitize_event.py | Evento de saneamento | Obsoleto | Mover para archive |

### Diretrios na Raiz (pr-saneamento)
| Diretrio | Descrio | Status | Ao Recomendada |
|-----------|-----------|--------|------------------|
| .agents/ | Regras e workflows antigos | Legado | Manter (pode conter histrico) |
...
| .ruff_cache/ | Cache do ruff | Temporrio | Excluir (pode ser recriado) |

### Arquivos em backup_root/
| Arquivo | Descrio | Ao |
|---------|-----------|------|
| CHEATSHEET.md | Cheatsheet do projeto | Mover para docs |
| Sem ttulo.txt | Arquivo temporrio | Excluir |
| install.ps1 / install.sh | Scripts de instalao | Mover para archive |
| Vrios scripts .py | Utilitrios de desenvolvimento | Mover para archive |

## Plano de Saneamento

### Fase 1: Documentao
- [x] Criar Lobo LEGACY
- [x] Inventariar arquivos e diretrios
- [x] Definir aes para cada item

### Fase 2: Movimentao
- [ ] Mover arquivos .py utilitrios para DIR-ARC-FR-001-archive-main
- [ ] Mover contedo de backup_root para DIR-BAK-FR-001-backup-main
- [ ] Organizar white_label dentro da estrutura do framework
- [ ] Preservar .agents/ como histrico

### Fase 3: Limpeza
- [ ] Remover arquivos .py da raiz
- [ ] Remover backup_root/ vazio
- [ ] Limpar .ruff_cache/
- [ ] Verificar integridade aps movimentao

### Fase 4: Validao
- [ ] Verificar se framework ainda funciona
- [ ] Atualizar referncias se necessrio
- [ ] Criar checkpoint final

## Notas Importantes

1. **Preservao de Histrico**: Arquivos legados so movidos, no excludos
2. **Estrutura do Framework**: Usar diretrios existentes do NeoCortex:
   - DIR-ARC-FR-001-archive-main: Para arquivos utilitrios obsoletos
   - DIR-BAK-FR-001-backup-main: Para backups antigos
   - DIR-DOC-FR-001-docs-main: Para documentao legada
3. **White Label**: O diretrio white_label/ na raiz ser mantido como template
4. **Exemplos**: O diretrio examples/ ser mantido para referncia
...
---
*Este Lobo ser atualizado durante o processo de saneamento.*
"""
    )
...
def inventory_root_files():
    """Faz inventrio de arquivos na raiz."""
...
def move_utility_files():
    """Move arquivos utilitrios para o archive do framework."""
    print("\n3. Movendo arquivos utilitrios para archive...")
    ...
    for filename, target_dir, description in files_to_move:
        ...
        try:
            ...
            # Remover da raiz (opcional - comentado por segurana)
            # source.unlink()
...
def update_legacy_lobe(lobe_name, moved_files, failed_files):
    ...
    if not lobe_info.get("exists"):
        print(f"   [ERROR] Lobe '{lobe_name}' no encontrado")
        return

    content = lobe_info["content"]

    # Adicionar seo de resultados
    results_section = f"""

## Resultados do Saneamento

### Arquivos Movidos ({len(moved_files)})
| Arquivo | Destino | Tamanho | Descrio |
|---------|---------|---------|-----------|
"""
...
### Estado Ps-Saneamento
- **Data concluso**: {datetime.now().strftime("%Y-%m-%d %H:%M:%S")}
- **Arquivos movidos**: {len(moved_files)}
- **Falhas**: {len(failed_files)}
- **Prximas aes**: Remoo segura dos arquivos da raiz aps validao

### Checkpoints Concludos
- [x] CP-LEGACY-001: Criao inicial do Lobo LEGACY
- [x] CP-LEGACY-002: Inventrio de arquivos legados
- [x] CP-LEGACY-003: Movimentao para archive
- [ ] CP-LEGACY-004: Limpeza da raiz (pendente validao)
"""

    # Adicionar ao contedo
...
def create_checkpoint():
    ...
    checkpoint_result = checkpoint_service.set_current_checkpoint(
        ...
        description="Saneamento da raiz do projeto: movimentao de arquivos legados para archive",
...
def main():
    ...
    try:
        ...
        print(f"- Diretrios processados: {len(dirs)}")

        if failed:
            print("\n  ATENO: Alguns arquivos falharam ao mover:")
            for f in failed:
                print(f"   - {f['file']}: {f['error']}")

        print("\nPrximos passos:")
        print("1. Verificar se os arquivos foram movidos corretamente")
        print("2. Remover arquivos da raiz aps validao (opcional)")
        print("3. Revisar estrutura do archive")
        print("4. Atualizar referncias se necessrio")
