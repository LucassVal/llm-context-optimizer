...
class LedgerService:
    ...
    def prune_context(self) -> Dict[str, Any]:
        ...
        Moves oldest interactions from hot_context to cold_storage based on
        system_constraints.hot_context_limit.
        ...
        # Ensure memory_temperature structure exists
        if "memory_temperature" not in ledger:
            ledger["memory_temperature"] = {
                "hot_context": {"interactions": []},
                "cold_storage": {"archived_interactions": []},
                "compaction_log": {"entries": []},
            }

        memory = ledger["memory_temperature"]
        hot = memory.get("hot_context", {}).get("interactions", [])
        cold = memory.get("cold_storage", {}).get("archived_interactions", [])

        # Get limit from system constraints
        constraints = ledger.get("system_constraints", {})
        limit = constraints.get("hot_context_limit", 5)

        if len(hot) <= limit:
            return {
                "success": True,
                "message": f"No pruning needed: {len(hot)} hot interactions ≤ limit {limit}",
                "pruned_count": 0,
            }

        # Calculate how many to move
        to_move = len(hot) - limit
        moved = hot[:to_move]
        ...
        # Write back
        memory["hot_context"]["interactions"] = new_hot
        memory["cold_storage"]["archived_interactions"] = new_cold

        # Add entry to compaction log
        if "compaction_log" not in memory:
            memory["compaction_log"] = {"entries": []}

        memory["compaction_log"]["entries"].append(
            {
                "timestamp": datetime.utcnow().isoformat() + "Z",
                "action": "prune_context",
                "moved_count": len(moved),
                "hot_before": len(hot),
                "hot_after": len(new_hot),
                "cold_before": len(cold),
                "cold_after": len(new_cold),
            }
        )
        ...
        if success:
            return {
                "success": True,
                "message": f"Pruned {len(moved)} interactions from hot to cold storage",
                "pruned_count": len(moved),
                "hot_remaining": len(new_hot),
                "cold_total": len(new_cold),
            }
        else:
            return {"success": False, "error": "Failed to write ledger after pruning"}
